...
# Have to turn this on to write "notable dates" to a file
writeNotableDates = False
...
#      color = (0.25, 0.25, 0.25, 0.75)
    ...
    #  plt.text(sleepTimes[i][0], 0, sleepTimes[i][0].strftime("%m-%d %H:%M"),
      if sleepTimes[i][0] > startTime:
        ...
        plt.text(startTime + (endTime - startTime) / 140, maxLim - 0.4 * (i - startIndex) - 0.5,
                 "{} {}    {}".format(sleepTimes[i][0].strftime("%m-%d %H:%M"), sleepTimes[i][1], logStr),
                  fontdict={"size": "small", "fontweight": "bold"})
      ...
      if i < len(sleepTimes) - 1 and sleepTimes[i + 1][0] <= endTime:
        ...
        plt.text(startTime + (endTime - startTime) / 140, maxLim - 0.4 * (i + 1 - startIndex) - 0.5,
                 "{} {}   {}".format(sleepTimes[i + 1][0].strftime("%m-%d %H:%M"), sleepTimes[i + 1][1], logStr),
                  fontdict={"size": "small", "fontweight": "bold"})
